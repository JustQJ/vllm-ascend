...
#include "tensor_api/include/tensor_api/tensor.h"
...
struct CopyL12L0MxScaleA3510 {
    ...
    __aicore__ inline static void Copy(const T& dst, const U& src, const Coord& coord)
    {
        ...
        // Zz -> Zz
        uint16_t mStartPosition = Blaze::Gemm::CeilDiv(AscendC::Std::get<0>(coord), MATMUL_MNK_ALIGN);
        uint16_t kStartPosition = Blaze::Gemm::CeilDiv(AscendC::Std::get<1>(coord), 2);
