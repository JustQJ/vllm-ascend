...
TORCH_LIBRARY_EXPAND(CONCAT(_C, _ascend), ops)
{
    ...
    ops.impl("turboquant_sparse_flash_attention", torch::kPrivateUse1, &vllm_ascend::turboquant_sparse_flash_attention<false>);
    ops.def(
        "turboquant_sparse_flash_attention_grouped(Tensor query, Tensor key, Tensor value, Tensor sparse_indices, "
        "                                 Tensor? key_dequant_scale=None, Tensor? value_dequant_scale=None, *, "
        "                                 Tensor block_table, Tensor actual_seq_lengths_query, "
        "                                 Tensor actual_seq_lengths_kv, float scale_value, "
        "                                 int key_quant_mode, int value_quant_mode, int sparse_block_size, "
    ...
    ops.impl("turboquant_sparse_flash_attention_grouped", torch::kPrivateUse1, &vllm_ascend::turboquant_sparse_flash_attention<true>);
