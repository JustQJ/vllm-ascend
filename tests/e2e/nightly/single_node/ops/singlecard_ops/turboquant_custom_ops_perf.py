...
def bench_compress(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for num_tokens in args.tokens:
        ...
        stats = bench(
            lambda: torch.ops._C_ascend.turbo_quant_compress_latent(latent, centroids),
            args.warmup,
            args.active,
            args.inner,
        )
...
def bench_store(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for num_tokens in args.tokens:
        ...
        stats = bench(
            lambda: tq_latent_store.compress_kernel(latent, head_dim=TQ_HEAD_DIM),
            args.warmup,
            args.active,
            args.inner,
        )
...
def make_sfa_inputs(
    ...
    block_table = torch.stack(
        [torch.randperm(pool_blocks, generator=g)[:logical_blocks] for _ in range(batch)]
    ).to(torch.int32)
...
def bench_sfa(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for case in args.sfa_cases:
        ...
        flops = 2 * batch * q_tokens * q_heads * topk * TQ_COMBINE_DIM + 2 * batch * q_tokens * q_heads * topk * TQ_HEAD_DIM
