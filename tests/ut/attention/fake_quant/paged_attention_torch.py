...
``tests.ut.attention.fake_quant.paged_attention_npu.paged_attention`` so tests
can swap the Triton implementation for this golden path without reshaping
inputs.  It reconstructs per-sequence K/V from the paged cache and block table,
then runs the same TND attention math as the local FIA v2 golden tests.
"""
