...
def to_mxfp4(tensor, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    """Quantize tensor to MXFP4 (Microscaling FP4) with block-shared scale.

    MXFP4: 2 exponent bits + 1 sign bit + 2 mantissa bits (mbits=3 with implicit leading 1).
    Shared scale is computed per 32-element block.
    Reference: attn_block_mask_fix_hybird_mxfp4_offload.py
    """
    ...
    # Clip to max norm and handle special values
...
if __name__ == "__main__":
    ...
    test_op(1, 2, 256, 256, causal=False, dtype=torch.bfloat16, BM=32, BN=256)#failed
